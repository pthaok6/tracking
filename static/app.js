let orders = [];
let currentIndex = null;

// ===== LOAD từ server =====
async function loadOrders() {
    let res = await fetch("/orders");
    orders = await res.json();
    renderList();
}

// ===== SAVE về server =====
async function saveOrders() {
    await fetch("/orders", {
        method: "POST",
        headers: {"Content-Type": "application/json"},
        body: JSON.stringify(orders)
    });
}

// ===== ADD =====
async function add() {
    let code = document.getElementById("code").value;
    let note = document.getElementById("note").value;

    await fetch("/orders/add", {
        method: "POST",
        headers: {"Content-Type": "application/json"},
        body: JSON.stringify({code, note})
    });

    loadOrders();

    // clear input
    document.getElementById("code").value = "";
    document.getElementById("note").value = "";
}

// ===== DELETE =====
async function remove(i) {
    if (!confirm("Xoá đơn này?")) return;

    const el = document.querySelectorAll(".order")[i];

    // animation
    el.classList.add("removing");

    // đợi animation xong
    setTimeout(async () => {
        if (currentIndex === i) {
            currentIndex = null;
        } else if (currentIndex > i) {
             currentIndex--; // 👈 rất quan trọng
}
        // xoá local trước (để UI mượt)
        orders.splice(i, 1);
        renderList();

        // reset view nếu cần
        if (currentIndex === i) {
            document.getElementById("detail").innerHTML = "";
            document.getElementById("title").innerText = "Select an order";
            currentIndex = null;
        }

        // gọi API sau
        await fetch("/orders/delete", {
            method: "POST",
            headers: {"Content-Type": "application/json"},
            body: JSON.stringify({index: i})
        });

    }, 250);
}

// ===== RENDER LIST =====
function renderList() {
    let html = "";

    orders.forEach((o, i) => {
    
        let isActive = i === currentIndex;

        let isNew = o.last_time && (
            !o.seen_time || o.last_time > o.seen_time
        );
        html += `
        <div class="order ${i === currentIndex ? 'active' : ''}" onclick="view(${i})">
            
            <div class="order-content">
                <b>${o.note || "Không tên"}</b><br>
                <small>${o.code}</small>
            </div>

            <!-- delete button -->
            <div class="delete-btn" onclick="event.stopPropagation(); remove(${i})">
                <svg viewBox="0 0 24 24">
                    <path d="M6 6l12 12M18 6l-12 12"
                        stroke="currentColor" stroke-width="2" fill="none"/>
                </svg>
            </div>

        </div>`;
    });

    document.getElementById("list").innerHTML = html;
}

// ===== VIEW DETAIL =====
async function view(index) {
    currentIndex = index;

    let order = orders[index];

    document.getElementById("title").innerText =
        `📦 ${order.note || ""} (${order.code})`;

    let res = await fetch("/track_one", {
        method: "POST",
        headers: {"Content-Type": "application/json"},
        body: JSON.stringify({ tracking_number: order.code })
    });

    let data = await res.json();
    
    // 👉 update seen_time
    if (data.length > 0) {
     let latest = data[0].actual_time;
     order.last_time = latest;
     order.seen_time = latest;
}

    renderDetail(data);

// delay nhẹ để browser kịp apply state
    setTimeout(() => {
      renderList();
}, 50);
}

// ===== FORMAT TIME =====
function formatTime(epoch) {
    return new Date(epoch * 1000).toLocaleString();
}

// ===== RENDER TIMELINE =====
function renderDetail(records) {
    let html = "";

    records.forEach((r, i) => {
        let isLatest = (i === 0);

        html += `
        <div class="item ${isLatest ? 'latest' : ''}">
            <div class="time">${formatTime(r.actual_time)}</div>
            <div class="title">${r.buyer_description}</div>
        </div>`;
    });

    document.getElementById("detail").innerHTML = html;
}

// ===== AUTO REFRESH =====
setInterval(() => {
    if (currentIndex !== null) {
        view(currentIndex);
    }
}, 15000);

// ===== INIT =====
loadOrders();
